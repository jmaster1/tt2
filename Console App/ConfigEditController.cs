...
internal class ConfigEditController(
    IConfigRepository configRepository, 
    GameConfiguration config) : AbstractController
{
    ...
    public void Run()
    {
        Menu(Header,
            MenuItem("N <name>", 
                () => $"Name ({_config.Name})", 
                OnUpdateName),
            MenuItem("B <width> <height>", 
                () => $"Board size ({_config.BoardWidth} x {_config.BoardHeight})", 
                OnUpdateBoardSize),
            MenuItem("G <width> <height>", 
                () => $"Grid size ({_config.GridWidth} x {_config.GridHeight})", 
                OnUpdateGridSize),
            MenuItem("G <x> <y>", 
                () => $"Grid initial position ({_config.GridX} x {_config.GridY})", 
                OnUpdateGridPos),
            MenuItem("P <n>", 
                () => $"Piece count for each player ({_config.PlayerPieceCount})", 
                OnUpdatePlayerPieceCount),
            MenuItem("M <n>", 
                () => $"Number of moves to make before move pieces or grid available ({_config.MovePieceAfterNMoves})", 
                OnUpdateMovePieceAfterNMoves),
            MenuItem("W <n>", 
                () => $"Number of pieces in a row for a win ({_config.WinSequence})", 
                OnUpdateWinSequence),
